...
export class ProductsController {
  ...
  async create(@Body() createProductDto: CreateProductDto) {
    const existe = await this.productsService.findOne(
      createProductDto.id_producto,
    );
  ...
  async aplicarDescuento(
    ...
    if (user.role !== 'ADMIN') {
      throw new ForbiddenException(
        'Solo los administradores pueden agregar descuento',
      );
    }
  ...
  async asignaCateoria(
    ...
    if (user.role !== 'ADMIN') {
      throw new ForbiddenException(
        'Solo los administradores pueden asignar una categoria',
      );
    }
  ...
  async listaProductoPorCategoria(@Param('category') category: string) {
    const listaProductos =
      await this.productsService.obtenerProductosPorCategoria(category);
